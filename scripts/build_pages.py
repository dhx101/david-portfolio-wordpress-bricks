...
.study-card,.job-card{display:grid;grid-template-columns:260px 1fr;gap:24px;padding:24px}
@media (max-width:767px){.study-card,.job-card{grid-template-columns:1fr}}
.study-card-image,.job-card-image{aspect-ratio:4/3;overflow:hidden;border-radius:16px}
.study-card-image img{width:100%;height:100%;object-fit:cover;display:block}
.job-card-image{background:#f4f4f2;display:flex;align-items:center;justify-content:center;padding:24px}
.job-card-image img{width:100%;height:100%;object-fit:contain;display:block}
